...
type
  ...
  TKCMColorMap = array[0..255] of array[0..255] of array [0..2] of byte;
  ...
  TKCMMap = array[0..255] of array[0..255] of array[0..2] of integer; //Value1,Value2,Color(RGB)
  ...
  public
    ...
    property Map          :TKCMMap  read nKCMMap          write nKCMMap;
    //Loading functions
    procedure LoadFromFile(FileLocation:String; Patch:Integer);
    procedure SaveToFile(FileLocation:String);
...
{ -------------- KCM -------------- }
{ --------------------------------- }
procedure TKalClientMap.LoadFromFile(FileLocation:String; Patch:Integer);
var
  i:Integer;
  Reader:TReader;
begin
...
begin
  try
    for Y := 256 downto 0 do begin for X := 0 to 256 do begin map[x][y] := Reader.Readword(); end; end;
    Self.HeightMap:=Map;
  except
...
begin
  ...
  Writer.Position:=52;
  for U:=0 to 6 do  // Fix Changed from 7 to 6 because only 7 Layers are present
  begin
...
function TKalClientMap.ValuesToColor(Value1,Value2:DWORD):Integer;
begin
  If Value1=65535  then
  begin
    Case Value2 of
...
function TKalClientMap.ColorToValues(Color:Integer):TValues;
begin
  If GetRValue(Color)=255 then
  begin
    result[1]:=65535;
